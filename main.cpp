...
unsigned char* md5(std::ifstream *in)
{
...
char buf;
unsigned int A = 0x67452301, B = 0xefcdab89, C = 0x98badcfe, D = 0x10325476, AA, BB, CC, DD;
unsigned int T[64];
...
unsigned char *MD5 = md5(&in);
for(int i = 0; i < 16; i++)
out<<std::hex<<std::uppercase<<(unsigned int)MD5[i];
in.close();
out.close();
return 0;
}
